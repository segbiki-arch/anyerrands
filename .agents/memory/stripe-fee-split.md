...
## Pay button is NOT gated on isRequester — null-owner errands are payable by anyone
The frontend "Pay €X" button (errands/[id].tsx) renders for ANY logged-in viewer whenever `status==='accepted' && budgetAmount>0 && paymentStatus!=='paid'` — there is NO `isRequester` check around it. For owned errands the checkout route still blocks non-owners (`errand.requesterUserId && !== req.user.id`), but for errands with `requesterUserId = null` (anonymous posts AND demo/seed placeholders) checkout ALLOWS payment and binds the payer as requester (`requesterUserId: errand.requesterUserId ?? req.user.id`). **Consequence:** any placeholder/demo errand that has a budget and gets accepted can take real money from a random logged-in neighbour. **Rule:** seed/demo errands MUST have NO budget (budgetAmount null) — that's the only thing keeping the pay button hidden and money out. Do not give demo errands a budget unless you also add a real non-payable guard (e.g. an isDemo flag checked in /stripe/checkout).
